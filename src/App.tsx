...
import { LevelsPanel, OptionsPanel, OverviewPanel, ProfilingPanel, SystemPanel } from './components/panels'
import { Octicon } from './Octicon'
import { Report } from './Report'

const panels = ['Overview', 'System', 'Options', 'Levels', 'Profiling']
...
export function App() {
	...
	return <main onDrop={onDrop} onDragOver={e => e.preventDefault()}>
		{reports.length === 0 ? <>
			...
			<ul class="panels">
				{panels.filter(p => !(activeReport.server === undefined && p === 'Levels')).map((panel) => (
